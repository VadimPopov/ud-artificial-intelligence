...
class PlanningGraph():
    ...
    def add_action_level(self, level):
        ...
        for action in self.all_actions:
            # check if all of the action's prerequisite literals hold in the preceding S level
            if PgNode_a(action).prenodes <= self.s_levels[level]:
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        return (set(node_a1.action.effect_rem) & set(node_a2.action.precond_pos)) or \
    ...
    def h_levelsum(self) -> int:
        ...
        """
        level_sum = 0
        goal_fluents = self.problem.goal[:]
        for idx, level in enumerate(self.s_levels):
            # break out of the outer loop if all the goal fluents are achieved
            if not goal_fluents:
                break
            level = list(level)
            symbols = list(map(lambda n: n.symbol, level))
            for g in self.problem.goal:
                # break out of the inner loop if all the goal fluents are achieved
                if not goal_fluents:
                    break
                if g in symbols:
                    if g not in goal_fluents:
                        continue
                    level_sum += idx
                    goal_fluents.remove(g)
                    break
        return level_sum
