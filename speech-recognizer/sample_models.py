...
def deep_rnn_model(input_dim, units, recur_layers, output_dim=29):
    ...
    for i in range(recur_layers):
        simp_rnn = SimpleRNN(units, activation='relu',
        return_sequences=True)(input_data if i == 0 else bn_rnn)
        bn_rnn = BatchNormalization()(simp_rnn)
    # Add a TimeDistributed(Dense(output_dim)) layer
    time_dense = TimeDistributed(Dense(output_dim))(bn_rnn)
    # Add softmax activation layer
...
def bidirectional_rnn_model(input_dim, units, output_dim=29):
    ...
    bidir_rnn = Bidirectional(SimpleRNN(units, activation='relu',
        return_sequences=True, name='bidir_rnn'))(input_data)
    # Add a TimeDistributed(Dense(output_dim)) layer
...
def final_model(input_dim, filters, kernel_size, conv_stride,
    ...
    for i in range(recur_layers):
        bidir_rnn = Bidirectional(SimpleRNN(units, activation='relu',
        return_sequences=True, dropout=.3))(bn_cnn if i == 0 else bn_rnn)
